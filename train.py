...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gaussians.set_gaussian_mask(viewpoint_cam.camera_center, iteration, viewpoint_cam.resolution_scale)
        ...
        visit_mask = gaussians._gaussian_mask.clone()
        visit_mask[gaussians._gaussian_mask] = visibility_filter
        # Loss
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                gaussians.max_radii2D[visit_mask] = torch.max(gaussians.max_radii2D[visit_mask], radii[visibility_filter])
                gaussians.add_densification_stats(viewspace_point_tensor, visit_mask, visibility_filter)
                ...
                logger.info("\n[ITER {}] Densification complete".format(iteration))
                logger.info("\n[ITER {}] Clone Gaussians".format(gaussians.clone_gaussians))
                logger.info("\n[ITER {}] Split Gaussians".format(gaussians.split_gaussians))
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    scene.gaussians.set_gaussian_mask(viewpoint.camera_center, iteration, viewpoint.resolution_scale)
...
if __name__ == "__main__":
    ...
    if args.warmup:
        ...
        training(lp.extract(args), op.extract(args), pp.extract(args), dataset,  args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, wandb, logger, new_ply_path)
